...
contendo o tipo de token(\texttt{type}) e argumentos de função(\texttt{argIndex}), no caso de uma função definida.
...
O tamanho do identificador é \texttt{length}, e seus atributos são \texttt{type} e \texttt{argIndex}.
Com a excessão das funções pré-definidas, o atributo \texttt{argIndex} representa os nomes dos argumentos da função,
...
O atributo \texttt{argIndex} de um token é o índice da lista de seus parâmetros nesse membro.
